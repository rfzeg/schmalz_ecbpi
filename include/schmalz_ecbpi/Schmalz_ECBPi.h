...
public:
	...
	void downloadInitialSettings();	// from device
	void uploadInitialSettings(); // to device

	void downloadProfile(const uint8_t& i);
	void uploadProfile(const uint8_t& i);

	void downloadObservation();	// read-only

	void uploadCommand();
	void downloadState();

	int sysCmdFactoryReset();
	int sysCmdCalibrateSensor();
	int sysCmdCounterReset();
	int sysCmdVoltageReset();
	int sysCmdVacuumReset();

	//Internal
	schmalz_ecbpi::DeviceID getDeviceID(){return dev_ID_;};		// read only params from device

	void setInitialSettings(const schmalz_ecbpi::InitialSettings& init_settings){this->initial_settings = init_settings;};	//to private member
	schmalz_ecbpi::InitialSettings getInitialSettings(){return this->initial_settings;};
	
	void setProfile(const schmalz_ecbpi::Profile& p, const uint8_t& i){this->profiles[i] = p;};
	schmalz_ecbpi::Profile getProfile(const int& i){return this->profiles[i];}; 

	schmalz_ecbpi::Observation getObservation(){return this->full_Obs_;};

	void setCommand(const schmalz_ecbpi::Command& pdo){this->pd_Out_ = pdo;};
	schmalz_ecbpi::Command getCommand(){return this->pd_Out_;};

	schmalz_ecbpi::State getState(){return this->pd_In_;};

private:

	schmalz_ecbpi::DeviceID 	dev_ID_;
	schmalz_ecbpi::InitialSettings initial_settings;
	schmalz_ecbpi::Profile 		profiles[4];
	schmalz_ecbpi::State		pd_In_;			//Sensors value from device = process data in
	schmalz_ecbpi::Command		pd_Out_;		//Command from PC to device = process data out
	schmalz_ecbpi::Observation	full_Obs_;		//Observation and diagnosis
	SerialPort					ser_port_;


	int writeParameter(uint8_t address, uint8_t sub_index, uint8_t len, uint16_t value);
	int readParameter(uint8_t address, uint8_t sub_index, uint8_t len);
	std::string readStringParameter(uint8_t address, uint8_t sub_index, uint8_t len);

	void downloadDeviceID();

	void downloadProfile_0();
	void downloadProfile_1();
	void downloadProfile_2();
	void downloadProfile_3();

	void uploadProfile_0();
	void uploadProfile_1();
	void uploadProfile_2();
	void uploadProfile_3();
